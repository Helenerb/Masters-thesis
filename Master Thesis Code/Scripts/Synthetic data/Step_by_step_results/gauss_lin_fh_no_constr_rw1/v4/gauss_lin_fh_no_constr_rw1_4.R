...
investigation.name <- "gauss_lin_fh_no_constr_rw1"
...
run_stan(
  stan_program="Scripts/Synthetic data/Stan analyses/stan_programs/step_by_step_results/stan_gaus_lin_fh_rw1_no_constr.stan",
  obs = obs, chains=4, warmup = 8000, iter = 80000, output.path = stan.output,
  config.name = investigation.name, markov=F)

inlabru.gaus.lin.fh.rw1.no.constr <- function(obs, max_iter=30){
...
res.inlabru <- inlabru.gaus.lin.fh.rw1.no.constr(obs, max_iter = 100)
