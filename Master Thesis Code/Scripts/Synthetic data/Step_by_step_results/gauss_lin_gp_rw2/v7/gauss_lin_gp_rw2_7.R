...
investigation.name <- "gauss_lin_gp_rw2"
...
run_stan(
  stan_program="Scripts/Synthetic data/Stan analyses/stan_programs/step_by_step_results/stan_gaus_lin_gp_rw2_sc.stan",
  obs = obs, chains=4, warmup = 8000, iter = 80000, output.path = stan.output,
  config.name = investigation.name, markov=F)

inlabru.gaus.lin.gp.rw2 <- function(obs, max_iter=30){
  ...
  comp = ~ -1 +
    ...
    kappa(t, model = "rw2", hyper = loggamma.prior.high.variance, constr = TRUE)
...
res.inlabru <- inlabru.gaus.lin.gp.rw2(obs, max_iter = 100)
