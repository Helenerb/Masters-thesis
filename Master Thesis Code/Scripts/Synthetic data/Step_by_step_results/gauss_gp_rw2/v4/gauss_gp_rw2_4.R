...
run_stan(
  ...
  obs = obs, chains=4, warmup = 500000, iter = 600000, output.path = stan.output,
...
res.inlabru <- inlabru.gaus.gp.rw2(obs, max_iter = 100)
...
plots_compared <- produce.compared.plots(
  ...
  underlying.effects = underlying.effects,
  plot.func = function(...) {plot.inlabru.stan.traditional.lc(..., cohort=FALSE, tau.beta.cutoff = 700, tau.kappa.cutoff = 2000, tau.alpha.cutoff = 10, a45=F)},
