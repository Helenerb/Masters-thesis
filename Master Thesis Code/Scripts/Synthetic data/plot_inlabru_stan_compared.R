...
plot.inlabru.stan.compared.rw2 <- function(stan.summaries,
  ...
  # p.tau.alpha <- ggplot() + labs(title="Fixed hypers")
  # p.tau.beta <- ggplot() + labs(title="Fixed hypers")
  # p.tau.kappa <- ggplot() + labs(title="Fixed hypers")
  # p.tau.epsilon <- ggplot() + labs(title="Fixed hypers")
  # p.tau.gamma <- ggplot() + labs(title="Fixed hypers")
...
plot.inlabru.stan.compared.poisson.lc.fixed.hypers <- function(stan.summaries,
                                           stan.marginals,
                                           inlabru.summaries,
                                           res.inlabru,
                                           underlying.effects,
...
plot.inlabru.stan.traditional.lc.fixed.hypers <- function(stan.summaries,
                                             stan.marginals,
                                             inlabru.summaries,
                                             res.inlabru,
                                             underlying.effects,
  ...
  print("printing the true etas for both, to be sure they are the same")
  print("Inlabru: ")
...
plot.inlabru.stan.traditional.lc.fixed.hypers.no.beta <- function(stan.summaries,
                                                          stan.marginals,
                                                          inlabru.summaries,
                                                          res.inlabru,
                                                          underlying.effects,
...
plot.inlabru.stan.traditional.lc.fixed.hypers.no.intercept <- function(stan.summaries,
                                                          stan.marginals,
                                                          inlabru.summaries,
                                                          res.inlabru,
                                                          underlying.effects,
  ...
  p.intercept <- ggplot() + labs(title = "No intercept")
  ...
  print("printing the true etas for both, to be sure they are the same")
  print("Inlabru: ")
  print(inlabru.summaries$data.eta$true.eta)
  print("Stan: ")
  print(stan.summaries$summary_eta)
  print("Finish printing etas")
...
plot.inlabru.stan.poisson.lc.no.beta <- function(stan.summaries,
                                                     stan.marginals,
                                                     inlabru.summaries,
                                                     res.inlabru,
                                                     underlying.effects,
  ...
  p.beta <- ggplot() + labs(title = "Linear model, no beta")
  ...
  p.tau.beta <- ggplot() + labs(title = "No beta available")
...
save.compared.rw2 <- function(plots, path.to.storage, cohort=TRUE, pdf=T, png=F){
  if(cohort){
    p.random.effects <- (plots$p.intercept | plots$p.alpha | plots$p.beta )/(plots$p.kappa | plots$p.gamma) + 
      plot_layout(guides="collect")
  } else {
  ...
  save.figure(p.eta.xt, name = "eta_xt_comparison", path = path.to.storage, png=png, pdf=pdf)
  
  p.eta.facet <- (plots$p.eta.x | plots$p.eta.t) + plot_layout(guides = "collect") & theme(legend.position = "bottom")
  save.figure(p.eta.facet, name = "eta_facet_comparison", path = path.to.storage, png=png, pdf=pdf)
