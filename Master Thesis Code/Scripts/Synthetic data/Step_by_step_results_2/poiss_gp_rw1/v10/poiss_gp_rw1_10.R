...
investigation.path <- file.path(investigation.name, "v10")
output.path <- file.path("Scripts/Synthetic\ data/Step_by_step_results_2", investigation.path)
stan.output <- output.path  # deprecated

#   ----    Retrieve the data   ----

synthetic.male.lung.v10 <- function(){
  obs <- read.csv("Data/synthetic_male_lung_10.csv")
...
config.data <- synthetic.male.lung.v10()
obs <- config.data$obs
...
stan_fit <- run_stan(
  stan_program = file.path(output.path, "stan_poiss_gp_rw1_10.stan"),
