...
investigation.path <- file.path(investigation.name, "v9")
...
synthetic.male.lung.v9 <- function(){
  ...
  obs.trad <- obs %>% 
    ...
    mutate(x = x+1 - 2, x.c = x.c + 1 - 2, t = t + 1)
...
stan_fit <- run_stan(
  stan_program = file.path(output.path, "stan_poiss_gp_rw1_9.stan"),
  obs = obs, chains=4, warmup = 3000, iter = 30000, output.path = stan.output)
...
p.predictor.summary <- ggplot(data = data.frame(xt = obs$xt,
                                                mean = res.inlabru$summary.linear.predictor$mean[1:288],
                                                X0.025quant = res.inlabru$summary.linear.predictor$`0.025quant`[1:288],
                                                X0.975quant = res.inlabru$summary.linear.predictor$`0.975quant`[1:288])) + 
...
pred.32.inlabru <- data.frame(res.inlabru$marginals.linear.predictor$APredictor.032)

p.pred.32 <- ggplot(pred.32.inlabru) + 
  geom_area(aes(x = x, y = y, fill = "Inlabru", color = "Inlabru"), alpha = 0.5) + 
  geom_density(data = stan.predictor.df, aes(x = X32, fill = "Stan", color = "Stan"), alpha = 0.5) + 
  theme_classic() + 
  scale_color_manual(name = "", values = palette) + 
  scale_fill_manual(name = "", values = palette) + 
  labs(title = "Predictor at xt=32", x = "", y = "")
p.pred.32

save.figure(p.pred.32, name = "predictor_32", path = output.path, png= F)
...
p.tau.kappa <- ggplot() +
  geom_density(data=data.frame("x" = list_of_draws$tau_kappa) %>% filter(x < 100), aes(x = x, color = "Stan", fill = "Stan"), alpha = 0.2) +
  geom_area(data = data.frame(res.inlabru$marginals.hyperpar$`Precision for kappa`) %>% filter(x < 100), aes(x = x, y = y, color = "Inlabru", fill = "Inlabru"), alpha = 0.2) +
  theme_classic() +
  scale_color_manual(name = "", values = palette) +
  scale_fill_manual(name = "", values = palette) +
...
p.tau.eta <- ggplot() +
  ...
  geom_area(data = data.frame(res.inlabru$marginals.hyperpar$`Precision for epsilon`) %>% filter(x < 1500), aes(x = x, y = y, color = "Inlabru", fill = "Inlabru"), alpha = 0.2) +
  theme_classic() +
  scale_color_manual(name = "", values = palette) +
  scale_fill_manual(name = "", values = palette) +
