...
investigation.name <- "gauss_lin_gp_rw1_testing_simplified"
...
stan_fit  <- run_stan(
  stan_program=file.path(stan.output, "stan_gauss_lin_gp_rw1_simplified.stan"),
  obs = obs, chains=4, warmup = 16000, iter = 160000, output.path = stan.output,
...
inlabru.gaus.lin.gp.rw1 <- function(obs, max_iter=30){
  ...
  comp = ~ -1 +
    ...
    alpha(x, values = unique(obs$x), model = "rw1", hyper = loggamma.prior, constr = FALSE) +
    #beta(x.c, model = "iid", hyper = fixed.theta.beta, constr = FALSE) +
    kappa(t, values = unique(obs$t), model = "rw1", hyper = loggamma.prior.high.variance, constr = TRUE)
  
  #formula = eta ~ alpha + kappa
  formula = eta ~ .
...
p.pred.54 <- ggplot() + 
  geom_area(data = data.frame(res.inlabru$marginals.linear.predictor$APredictor.054), aes(x = x, y = y, color = "Inlabru", fill = "Inlabru"), alpha = 0.2) + 
  geom_density(data = data.frame("x" = data.frame(eta_draws)$X54), aes(x = x, color = "Stan", fill = "Stan"), alpha = 0.2) +
  theme_classic() + 
  scale_color_manual(name = "", values = palette) + 
  scale_fill_manual(name = "", values = palette) + 
  labs(title = "Predictor[54]", x = "", y = "")

ggsave("pred_54.pdf", p.pred.54, path=output.path, device = "pdf", dpi="retina", height = 5, width = 8)
...
p.theta <- (p.theta.alpha | p.theta.kappa)/(p.theta.eta) + plot_layout(guides = "collect")
...
p.tau.h <- (p.tau.alpha.h | p.tau.kappa.h)/(p.tau.eta.h) + plot_layout(guides = "collect")
p.theta.h <- (p.theta.alpha.h | p.theta.kappa.h)/(p.theta.eta.h) + plot_layout(guides = "collect")
