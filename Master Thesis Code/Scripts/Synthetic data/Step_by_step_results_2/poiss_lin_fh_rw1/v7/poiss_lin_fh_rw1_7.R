...
stan_fit <- run_stan(
  stan_program = file.path(output.path, "stan_poiss_lin_fh_rw1_7.stan"),
...
inlabru.poiss.lin.fh.rw1 <- function(obs, output.path, max_iter=30){
  ...
  comp = ~ -1 +
    alpha(x, model = "rw1", hyper = fixed.theta.alpha, constr = FALSE, scale.model = F) +
    #beta(x.c, model = "iid", hyper = fixed.theta.beta, constr = FALSE) +
    kappa(t, model = "rw1", hyper = fixed.theta.kappa, constr = TRUE, scale.model = F) + 
...
res.inlabru <- inlabru.poiss.lin.fh.rw1(obs, output.path = output.path, max_iter = 100)
