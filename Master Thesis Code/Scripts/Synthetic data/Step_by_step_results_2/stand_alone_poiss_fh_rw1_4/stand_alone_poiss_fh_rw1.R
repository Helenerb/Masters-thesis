...
#' Poisson model, with fixed priors, with kappa and alpha as rw1s. 
#' 
...
investigation.name <- "stand_alone_poiss_fh_rw1_4"  # TODO: change!
...
stan.fit <- run_stan(
  stan_program="stan_pois_fh_rw1_sc_4.stan",
...
plot.inlabru.vs.underlying.lc.fixed.hypers <- function(
  res.inlabru, underlying.effects, path.to.storage="", pdf=T, png=F, save=F){
...
plot.inlabru.stan.lc.fixed.hypers <- function(stan.summaries,
                                              stan.marginals,
                                              inlabru.summaries,
                                              res.inlabru,
                                              underlying.effects
...
plots.summaries.inlabru <- plot.inlabru.vs.underlying.lc.fixed.hypers(
  res.inlabru,
  underlying.effects,
...
plots_compared <- produce.compared.plots(
  ...
  underlying.effects = underlying.effects,
  plot.func = plot.inlabru.stan.lc.fixed.hypers,
