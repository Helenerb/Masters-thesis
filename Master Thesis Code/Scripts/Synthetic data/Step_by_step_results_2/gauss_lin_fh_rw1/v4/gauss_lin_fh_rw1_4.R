...
stan_fit <- run_stan(
  stan_program = file.path(output.path, "stan_gauss_lin_fh_rw1.stan"),
  obs = obs, chains=4, warmup = 500, iter = 5000, output.path = stan.output)
...
res.inlabru <- inlabru.gaus.lin.gp.rw1(obs, output.path = output.path, max_iter = 100)
