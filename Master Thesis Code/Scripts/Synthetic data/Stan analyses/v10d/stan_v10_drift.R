...
run_stan_v10d <- function(stan_program, chains=4, warmup=1000, iter=10000, markov=TRUE){
  output.path <- set_workspace(config="v10d", markov)
  ...
  store_stan_results(fit=stan_fit, output.path=output.path, config="v10d")
}

run_stan_v10d(stan_program="Stan\ analyses/stan_programs/stan_analysis_lc_rw_drift.stan", chains=4, warmup = 4000, iter = 40000, markov=TRUE)
