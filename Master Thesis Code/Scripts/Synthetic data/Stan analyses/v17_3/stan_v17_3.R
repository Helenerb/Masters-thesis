# running the v17_3 configuration with the stan program where
...
run_stan_v17_3 <- function(stan_program, chains=4, warmup=1000, iter=10000, markov=TRUE){
  output.path <- set_workspace(config="v17_3", markov)
  source("run_stan_functions.R")
  
  source("configurations_synthetic_data.R")
  
  data = configuration.v17.3()
...
run_stan_v17_3(stan_program="Stan\ analyses/stan_programs/stan_analysis_cohort_rw2.stan", chains=4, warmup = 5000, iter = 50000, markov=TRUE)
