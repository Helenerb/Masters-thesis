# running the v17 configuration with the stan program where
...
run_stan_v17dh <- function(stan_program, chains=4, warmup=1000, iter=10000, markov=TRUE){
  output.path <- set_workspace(config="v17dh", markov)
  source("run_stan_functions.R")
  
  source("configurations_synthetic_data.R")
  
  data = configuration.v17.1()
  
  stan_fit <- run_stan_program_lcc(data, chains=chains, warmup=warmup, iter=iter, stan_program=stan_program)
  
  store_stan_results(fit=stan_fit, output.path=output.path, config="v17dh")
}

run_stan_v17dh(stan_program="Stan\ analyses/stan_programs/stan_analysis_lcc_rw_drift_h.stan", chains=4, warmup = 4000, iter = 40000, markov=TRUE)
