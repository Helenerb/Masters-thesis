// This is the fourth attempt at successful model implementatioj in STAN. 
...
// This STAN program is run in the R script comparison_inlabru_stan_lc

// Here, STAN is implemented with alpha as unconstrained iid. 
...
transformed parameters {
  ...
  vector[X*T] eta = alpha[x] + beta[x].*kappa[t] + (phi*ts).*beta[x] + epsilon;
}
...
model {
  ...
  target += normal_lpdf(alpha | 0, 1/sqrt(tau_alpha));
  target += normal_lpdf(beta_raw | 0, 1/sqrt(tau_beta));
