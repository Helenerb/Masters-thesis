...
set_workspace <- function(markov=TRUE){
  if(markov){
    ...
    setwd("~/Documents/GitHub/Masteroppgave/Masters-thesis/Master Thesis Code/Scripts/Synthetic data/Random walk implementations/Rw1_poisson_testing/inla_inlabru/sin_z")
  } else {
    setwd("~/Desktop/Masteroppgave/Masters-thesis/Master Thesis Code/Scripts/Synthetic data/Random walk implementations/Rw1_poisson_testing/inla_inlabru/sin_z")
  }
}
...
run.inlabru.fh <- function(data, E, initial = log(75)){
  ...
  components = ~ -1 + Int(1) + eta(x, model = "rw1", constr = TRUE, hyper = hyper.eta, scale.model = T)
...
run.inlabru.pc <- function(data, E){
  ...
  hyper.eta <- list(prec = list(prior = "pc.prec", param = c(1, 0.01)))
...
run.inla.pc <- function(data, E, initial = log(75)){
  hyper.eta.inla <- list(prec = list(prior = "pc.prec", param = c(1, 0.01)))
  formula  = y ~ f(x, model = "rw1", constr = TRUE, hyper = hyper.eta.inla, scale.model = T)
  result <- inla(formula, family  ="poisson", data = data, E = E)
...
p.inla_inlabru.pc <- ggplot() + 
  ...
  theme_classic() + 
  scale_color_manual(name = "", values = palette) + 
  labs(title = "Comparison between Inlabru and INLA, pc priors, param = (1, 0.01)", x = "", y = "")
...
run.inlabru.default <- function(data, E){
  ...
  components = ~ Intercept + eta(x, model = "rw1", constr = TRUE, scale.model = T)
