...
setwd("~/Desktop/Masteroppgave/Masters-thesis/Master Thesis Code/Scripts/Synthetic data/Random walk implementations/Rw1_testing_stand_alone")
...
p.theta_eta_ib <- ggplot(tau_eta_df)  + 
  geom_density(aes(x = Diff.1.theta, color = "Stan", fill = "Stan"), alpha = 0.3) + 
...
p.intercept <- ggplot() + 
  geom_density(data = data.frame(Diff.1 = list_of_draws_diff_1$intercept), aes(x = Diff.1, color = "Stan", fill = "Stan"), alpha = 0.5) + 
