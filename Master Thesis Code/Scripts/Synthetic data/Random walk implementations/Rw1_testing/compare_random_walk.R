...
set_workdirectory <- function(markov=TRUE){
  if(markov){
    ...
    setwd("~/Documents/GitHub/Masteroppgave/Masters-thesis/Master Thesis Code/Scripts/Synthetic data/Random walk implementations/Rw1_testing")
  } else {
    setwd("~/Desktop/Masteroppgave/Masters-thesis/Master Thesis Code/Scripts/Synthetic data/Random walk implementations/Rw1_testing")
  }
}
...
p.theta_eta_ib <- ggplot(tau_eta_df)  + 
  geom_density(aes(x = Diff.1.theta, color = "Difference 1", fill = "Difference 1"), alpha = 0.3) + 
...
p.tau_eta_ib <- ggplot(tau_eta_df) + 
  geom_density(aes(x = Diff.1, color = "Difference 1", fill = "Difference 1"), alpha = 0.3) + 
...
p.tau_ib  <- (p.tau_eta_ib | p.tau_y_ib | p.theta_eta_ib) + plot_layout(guides = "collect")
...
p.summary.eta.ib <- ggplot(summary_eta, aes(x = idx)) + 
  geom_ribbon(aes(ymin = Diff.1.025, ymax = Diff.1.975, fill = "Difference 1", color = "Difference 1"), alpha = 0.5) + 
  ...
  geom_point(aes(y = Diff.1, color = "Difference 1", fill = "Difference 1")) + 
...
p.intercept <- ggplot() + 
  geom_density(data = data.frame(Diff.1 = list_of_draws_diff_1$intercept), aes(x = Diff.1, color = "Difference 1", fill = "Difference 1"), alpha = 0.5) + 
