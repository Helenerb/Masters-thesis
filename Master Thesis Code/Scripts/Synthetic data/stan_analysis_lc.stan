...
parameters {
  ...
  vector[X-1] alpha_raw;  //  before sum-to-zero is implemented
  vector[X-1] beta_raw;  //  before sum-to-unit is implemented
...
model {
  ...
  // prior distributions
  target += normal_lpdf(alpha[2:X]| alpha[1:X-1], 1/sqrt(tau_alpha));
  target += normal_lpdf(beta | 0, 1/sqrt(tau_beta));
  ...
  target += normal_lpdf(kappa[2:T] | kappa[1:T-1], 1/sqrt(tau_kappa));
