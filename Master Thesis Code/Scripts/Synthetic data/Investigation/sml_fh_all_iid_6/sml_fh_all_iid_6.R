...
investigation.name <- "sml_fh_all_iid_6"
...
run_stan(
  stan_program="Scripts/Synthetic\ data/Stan\ analyses/stan_programs/stan_lc_fh_iid.stan",
  obs = obs, chains=4, warmup = 2000, iter = 20000, output.path = stan.output,
  config.name = investigation.name, markov=F)

inlabru.lc.fh.iid<- function(obs, max_iter=30){
...
res.inlabru <- inlabru.lc.fh.iid(obs, max_iter = 150)
