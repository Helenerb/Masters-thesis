...
#' Only sum-to-one constraints on beta are imposed
...
investigation.name <- "tllp_fh_all_iid_beta_constr_4"
...
run_stan(
  stan_program="Scripts/Synthetic\ data/Stan\ analyses/stan_programs/stan_tllp_fh_iid_beta_constr.stan",
  obs = obs, chains=4, warmup = 2000, iter = 20000, output.path = stan.output,
  config.name = investigation.name, markov=F)


#   ----   Define and run inlabru   ----

inlabru.traditional.lc.fixed.hypers.all.iid.beta.constr <- function(obs, max_iter=30){
...
res.inlabru <- inlabru.traditional.lc.fixed.hypers.all.iid.beta.constr(obs, max_iter = 100)
