...
plot.inlabru.vs.underlying.lc.ar1c <- function(res.inlabru, underlying.effects,
                                                     path.to.storage="", save=FALSE,
                                                     phi.plus.kappa.func=phi.plus.kappa.v17){
  ...
  data.kappa = data.frame(cbind(mean = res.inlabru$summary.random$kappa$mean[1:nt],
                     `0.025quant` = res.inlabru$summary.random$kappa$`0.025quant`[1:nt],
                     `0.975quant` = res.inlabru$summary.random$kappa$`0.975quant`[1:nt],
                     kappa.true = underlying.effects$kappa.true[1:nt],
                     kappa.drifted = underlying.effects$kappa.true[1:nt] + underlying.effects$phi.true*(0:(nt-1)),
                     ID = 0:(nt-1)))
  
  p.kappa <- ggplot(data = data.kappa, aes(x = ID)) + 
    geom_ribbon(aes(ymin = X0.025quant, ymax = X0.975quant, fill = "Estimated"), alpha = 0.4) + 
    geom_point(aes(y = kappa.drifted, color = "True value", fill = "True value"), size = 0.5) + 
    geom_point(aes(y = mean, color = "Estimated", fill = "Estimated"), size = 0.5) + 
    scale_color_manual(name = "",
                       values = palette ) +
  ...
  data.drift <- data.frame(res.inlabru$marginals.random$kappa[nt+1]) %>%
    mutate(phi.true = underlying.effects$phi.true)
  
  p.drift <- data.drift %>% ggplot(aes(.[[1]], .[[2]])) + 
    geom_area(aes(fill = "Estimated", color = "Estimated"), alpha = 0.4) + 
    geom_vline(aes(xintercept = phi.true, fill = "True", color = "True")) + 
    scale_fill_manual(name = " ", values = palette) + 
    scale_color_manual(name = " ", values = palette) + 
    labs(x = "Value of phi", y = " ", title = "Drift term - inlabru")
  ...
  p.random.effects <- (p.intercept | p.alpha ) / (p.beta | p.kappa | p.drift ) + 
    plot_layout(guides = "collect")
  ...
  p.eta.all <- (p.eta | p.eta.2) + 
    plot_layout(guides = "collect") & theme(legend.position = "bottom")
  
  if(save){
    save.figure(p.eta.all, name="eta_xt_inlabru", path=path.to.storage)
  }
...
plot.inlabru.vs.underlying.lc.ar1c.2 <- function(res.inlabru, underlying.effects,
                                               path.to.storage="", save=FALSE,
                                               phi.plus.kappa.func=phi.plus.kappa.v17){
  ...
  data.kappa = data.frame(cbind(mean = res.inlabru$summary.random$kappa$mean[1:nt],
                                `0.025quant` = res.inlabru$summary.random$kappa$`0.025quant`[1:nt],
                                `0.975quant` = res.inlabru$summary.random$kappa$`0.975quant`[1:nt],
                                kappa.true = underlying.effects$kappa.drifted[1:nt],
                                ID = 0:(nt-1)))
...
plot.inlabru.vs.underlying.cohort.ar1c.2 <- function(res.inlabru, underlying.effects,
                                                 path.to.storage="", save=FALSE,
                                                 phi.plus.kappa.func=phi.plus.kappa.v17){
  ...
  p.random.effects <- (p.intercept | p.alpha | p.beta ) / (p.kappa | p.drift | p.gamma) + 
    plot_layout(guides = "collect")
  ...
  plots <- list(p.alpha = p.alpha, p.beta = p.beta, p.kappa = p.kappa,
                p.drift = p.drift, p.gamma = p.gamma, p.eta = p.eta,
                p.eta.2 = p.eta.2, p.eta.x = p.eta.x, 
                p.eta.t = p.eta.t, p.intercept = p.intercept,
                p.random.effects = p.random.effects,
                p.eta.all = p.eta.all,
