...
investigation.name <- "gauss_fh_rw2"
...
run_stan(
  stan_program="Scripts/Synthetic data/Stan analyses/stan_programs/step_by_step_results/stan_gaus_fh_rw2_sc_7.stan",
  obs = obs, chains=4, warmup = 8000, iter = 80000, output.path = stan.output,
  config.name = investigation.name, markov=F)

inlabru.gaus.fh.rw2 <- function(obs, max_iter=30){
  ...
  comp = ~ -1 +
    ...
    kappa(t, model = "rw2", hyper = fixed.theta.kappa, constr = TRUE)
...
res.inlabru <- inlabru.gaus.fh.rw2(obs, max_iter = 100)
