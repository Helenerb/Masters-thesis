...
\begin{equation}
    \tau_\alpha = 1.96, \tau_\beta = 202, \tau_\kappa = 336, \tau_\regepsilon = \tau_\eps = 420
    \label{eq:fixed_precisions_v4}
\end{equation}
and for the \vSeven data, the values are
\begin{equation}
    \tau_\alpha = 1.96, \tau_\beta = 64, \tau_\kappa = 336, \tau_\regepsilon = \tau_\eps = 420.
...
\begin{equation}
        M1: \quad m_{x,t} \sim \Normal(\xi_{x,t}, 1/\tau_{\epsilon}), \quad \xi_{x,t} = \mu + \alpha_x + \beta_x \cdot \kappa_t
    \label{eq:gaussian_model}
\end{equation}
\begin{equation}
    \begin{aligned}
        M2: \quad Y_{x,t} \sim \Poisson(E_{x,t}\cdot e^{\eta_{x,t}^{\text{lin}}}), \quad \eta_{x,t} = \mu + \alpha_x + \beta_x \cdot \kappa_t + \epsilon_{x,t}
    \end{aligned}
    \label{eq:poisson_model}
\end{equation}
with $\my$, $\alpha_x$, $\beta_x$, $\kappa_t$ and $\epsilon_{x,t}$ modeled as described in equation \ref{eq:rw1-random-effects} and with fixed values for $\tau_\alpha$, $\tau_\beta$, $\tau_\kappa$ and $\tau_\epsilon$. \textcolor{myDarkGreen}{Faktisk skriv dem ned. }

\subsubsection{Verification With Linear Models}
To check our implementation, and to ensure that \inlabru acts as we expect in the case with a regular linear predictor, we begin by running the comparison for linear versions of the model:
\begin{equation}
    \begin{aligned}
        M1_{\text{lin}}: \quad &m_{x,t} \sim \Normal(\xi_{x,t}^{\text{lin}}, 1/\tau_{\epsilon}) \\
        &\xi_{x,t}^{\text{lin}} = \mu + \alpha_x + \kappa_t
    \end{aligned}
    \label{eq:gaussian_lin}
\end{equation}
\begin{equation}
    \begin{aligned}
        M2_{\text{lin}}: \quad &Y_{x,t} \sim \Poisson(E_{x,t}\cdot e^{\eta_{x,t}^{\text{lin}}}) \\
        &\eta_{x,t}^{\text{lin}} = \mu + \alpha_x + \kappa_t + \epsilon_{x,t}
    \end{aligned}
    \label{eq:poisson_lin}
\end{equation}
with $\mu$, $\alpha_x$, $\kappa_t$ and $\epsilon_{x,t}$ as modelled in Equation \ref{eq:rw1-random-effects}. We run the models in \inlabru and \stan, for the two sets of data described in \textcolor{myDarkGreeen}{SECTION DESCRIBING SYNTHETIC DATA}. 
\begin{figure}
    \centering
    \textbf{M1lin: Linear Gaussian Model}
...
\begin{figure}
    \centering
    \textbf{M2lin: Linear Poisson Model}
